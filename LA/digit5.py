...
def extract_and_save(data, label, save_dir):
    for i, (x, y) in tqdm(enumerate(zip(data, label)), total=len(data)):
        if x.shape[2] == 1:
        ...
        # save to folder with label
        img_dir = osp.join(save_dir, "number " + str(y))
...
def main(data_dir):
    ...
    for name in datasets:
        ...
        train_dir = osp.join(data_dir, name)
